...
2. **Read shepherd.toml** — `.claude/shepherd.toml`. Apply `[paths]`, `[branching]`, `[ledger]`, `[mcp]`, `[cli]` throughout.
   - **Bootstrap path (#120):** If `.claude/shepherd.toml` does not exist, this is a first-ever plant. Surface the contents of `${CLAUDE_PLUGIN_ROOT}/examples/minimal/shepherd.toml`: *"No `.claude/shepherd.toml` found. Copy this template to `.claude/shepherd.toml`, then customize `[branching]`, `[gates]`, and `[paths]` (add a `[memory]` section if you keep project doctrines)."* Then STOP and require operator confirmation that the binding is written before continuing — do not derive config inline. The template provides safe defaults; the operator must make the branching + gate + path choices before seeding. Full procedure in `${CLAUDE_PLUGIN_ROOT}/agents/planter.md §Step 1`.
