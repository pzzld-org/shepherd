...
**Same probe, fuller record — the capability self-report (`CAPABILITY-SELF-REPORT`, DF-64/DF-65).** The check above answers ONE question (is `Workflow` present); this extends the IDENTICAL introspective act — reading your own visible tool list — to your WHOLE toolset, and makes the result durable instead of discarded after one branch decision. `agent_invocation_tagger.sh` already wrote a capability record for your dispatch at `<ns>/dispatch/<sprint>/<tool_use_id>.json` carrying `declared_tools` (from `agents/conductor.md:7`) plus an `observed_tools: null` placeholder — but `session_id` is NOT a usable key back to that specific record: `commands/spawn.md §Register teammates` documents that no caller ever learns a teammate's own session uuid, and this run's own registry confirms it live (`sqlite3 .shepherd/shepherd.db "select session_id from teammates"` → every row empty, DF-12/DF-71). Do not chase that record — write your OWN new one, keyed by nothing but what you already know (your role name, the current sprint):

```bash
role="conductor"; repo_root="$(git rev-parse --show-toplevel)"; sprint="$(git rev-parse --abbrev-ref HEAD)"
...
- `CAPABILITY-SELF-REPORT` — turn one completed without ever recording your OWN observed tool list into a dispatch capability record; required once per session, in the SAME probe as `WORKFLOW-VEHICLE-PROBE` (§Lane walk, DF-64/DF-65).
- `CAPABILITY-CONTAINMENT-BREACH` — your self-report found an OBSERVED tool that is both mutating and absent from your `tools:` declaration (e.g. `Edit`/`Write`/`Artifact`) — your read-only-to-source containment already failed; STOP, take no further write-shaped action, `SendMessage(to: root)` immediately (§Lane walk, DF-65).
