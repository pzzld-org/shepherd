...
- **WAVE-IMPL**: dispatch `@coder`(s) with the lane brief; they write to `INVOCATION-CONTEXT.worktree_path`. A coder worktree HEAD drifting off its base commit is `BASE-DRIFT` (`skills/shepherd/references/flock.md` §Write boundaries).
