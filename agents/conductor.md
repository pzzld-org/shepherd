...
- [ ] **CLOSE-FINALIZE** — mechanical procedure (like `.github/workflows/release.yml` handles patch→main, this handles dev.N→patch). Execute steps **in order**; do NOT skip or reorder. TEAMMATE mode: skip to step 7.
  ...
  **Step 9 — Adaptation loop** (SOLO mode only; #94/#95). Per `doctrines/adaptation-loop.md` + `doctrines/self-improvement.md`:
  1. **Record + harvest** (write) — once, before PAUSE:
     ...
     ```
     Writes one `sprint_metrics` row + harvests this sprint's HIGH/CRITICAL `audit_findings` into `mem_entries(kind='prior')`. `--wall-min` is auto-derived from the first sprint commit when omitted (so the cost-rising trend is never starved); pass it explicitly for precision. Provide `--api` when known — both power Check 8 (#94). Idempotent; on failure (DB locked) note under anomalies and continue — never block CLOSE-FINALIZE. **Supersedes** the retired completeness-auditor markdown append. The harvest echo (`N prior(s) harvested`) is the close report's "Learned" line — surface it.
