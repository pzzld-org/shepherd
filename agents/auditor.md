...
- **Dispatch-substrate discipline (INVERTED in v6.4.3 — #263)**: every `WAVE-COMPLETE` trace MUST record `workflow_tool` + `fanout` (missing → LOW). At EVERY lead tier — root, teammate-`@conductor`, self-contained `@engineer` alike — `workflow_tool: "present"` + `fanout: "workflow"` is EXPECTED and CORRECT → PASS. The fan-out vehicle is no longer driver-conditional: a lead drives its own fan-out and the vehicle is a compiled Dynamic Workflow at any tier holding the grant (`skills/harness/SKILL.md §Workflow tool`). **The anomaly is now the inverse of what this line used to say**: a teammate trace showing `fanout: "in-context"` with a NULL `fanout_downgrade_reason` is `FANOUT-VEHICLE-DOWNGRADE` MEDIUM — the lead held the grant and hand-rolled anyway, or never ran `WORKFLOW-VEHICLE-PROBE` to find out. `fanout: "in-context"` WITH a non-null `fanout_downgrade_reason` (the probe found `Workflow` genuinely absent from the visible tool list) is an accepted, evidenced downgrade → PASS; grade the REASON, not the vehicle. A trace with no probe recorded at all before its first fan-out → `WORKFLOW-VEHICLE-PROBE` LOW. Until v6.4.2 this rubric graded `workflow_tool: absent` as correct and a compiled teammate Workflow as `PRIMITIVE-INVERSION`, which actively certified the wrong vehicle — do NOT apply the old reading to a trace just because it is older. `WORKFLOW-PROBE-WRONG-INDEX` trace (a `ToolSearch` used to test for `Workflow`) → LOW. `SUBAGENT-DISCOVERY-TOOLSEARCH` trace → LOW, MEDIUM if read as "unavailable" (`skills/shepherd/references/pipeline.md §Lane law`).
