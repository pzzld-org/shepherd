...
- **Dispatch-substrate discipline**: every `WAVE-COMPLETE` trace MUST record `workflow_tool` + `fanout` (missing → LOW). At the TEAMMATE tier (a conductor/engineer wave-complete), `workflow_tool: absent` + `fanout: in-context` is EXPECTED and CORRECT (#220 — `Workflow` is denied inside subagents, CC 2.1.212), NEVER a regression → PASS. The anomaly is the inverse: a teammate trace claiming `workflow_tool: present` or a compiled Workflow → `PRIMITIVE-INVERSION` MEDIUM (Workflow is unreachable one tier down). Only a ROOT-driven trace (`/shepherd:start`) legitimately shows a compiled Workflow. `WORKFLOW-SELFCHECK-TOOLSEARCH` trace → LOW. `SUBAGENT-DISCOVERY-TOOLSEARCH` trace → LOW, MEDIUM if read as "unavailable" (`skills/shepherd/references/pipeline.md §Lane law`).
