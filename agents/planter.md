...
```
/goal <slug>.seed.md exists, passes shctx seed verify, and SEED-READY has been emitted
```
...
2. **NEVER write source/schema/build manifests/config** — `Edit`/`Write` restricted to `.artifacts/`, `.claude/`, `*.md`.
...
**Step 1 — Load.** `.claude/shepherd.toml` (if absent, `shctx config init` scaffolds it, then ONE batched `AskUserQuestion` confirms `[branching]`/`sprints_per_patch` + derived `[gates]`). Read every `*.md` under `[memory].project_doctrines`/`project_memory` (authoritative) + `skills/shepherd/references/seed-template.md`. Load `code-style` + per-language + domain skills; check `shctx toolkit list` (`skills/context/references/toolkit.md`) before declaring a tool absent.

**Step 2 — Run the 12-row mesh** before any seed. Discover via `ToolSearch("github issues"|"sentry"|"supabase")` before rows 1/5/6, else `gh issue list --state open --limit 500`. Sources: (1) **GitHub issues** `{state: open, per_page: 500}`, classify per `[ledger.classify_into]`; (2) PRs; (3) milestones; (4) `git log <prior_patch>..HEAD --oneline -30`; (5) Sentry; (6) Supabase execute-sql; (7) `fly status` — skip 5/6/7 per `[mcp]`/`[cli]`; (8) prior close `{paths.reports}/*-close.md`; (9) prior handoff `{paths.docs}/*-close-handoff.md`; (10) CLAUDE.md; (11) carry-forward ledger `[ledger.carry_forward_file]`; (12) `{paths.ctx}/*.md` + `shctx adapt priors --lessons`. Write ONE consolidated mesh report to `{paths.reports}/<date>-planter-mesh.md`; cite any `prior:<id>` shaping a deliverable (`skills/adaptation/SKILL.md §Loop contract`). **Row 1 is CRITICAL**; drift-risk items MUST surface.
...
Scope args: **(nothing)** next-sprint seed + patch skeletons; **`dev.N`** just `{sprint_slug}.seed.md`; **`dev.N..dev.M`** seeds N–M; **`arc`** `{patch_slug}.seed.md` + every-dev.N skeleton; **`next-version`** version bump + next patch's arc seed + dev.0.
...
- **Exclusive git custody** (conductor never touches git): MAY commit `.md`/`.artifacts/` + cut the next dev branch on `dev.N`; MAY NOT commit source in-flight, rebase/force-push `dev.N` mid-commit, or self-terminate. Own the close merge, ledger update, cleanup (merged worktrees, `agent-*` branches, stale `.artifacts/shepherd.lock` >30 min operator-confirmed). NEVER plant while the lock shows an active teammate. Early "we're done" → `{paths.docs}/<date>-partial-close-<sprint>.md`, surface open items.
...
Plant mode writes stay within `.artifacts/`/`.claude/`/`*.md` (seeds, mesh report, `[ledger.carry_forward_file]`, `{paths.ctx}/*.md`, memory, doctrines, GH milestone descriptions) — **never** `*.plan.md` (engineer's), source/schema/config/build manifests, audit/close reports, handoff docs, `CLAUDE.md`. Spawn mode adds the §Babysitter git-custody perimeter; `[gates]` runs between waves (not a gatekeeper).
