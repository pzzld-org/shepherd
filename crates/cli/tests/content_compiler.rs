...
fn codex_target_final_config_bytes_are_frozen() {
    ...
    //
    // `[models]` omits the leads entirely. `engineer` and `conductor` inherit
    // the caller, and an inherited hint resolves to no model line on Codex --
    // which is what inheriting MEANS there. Emitting a tier for them would pin
    // the thing the tier change exists to stop pinning.
