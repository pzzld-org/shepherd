...
**The fan-out vehicle stops being driver-conditional. Every tier that holds the `Workflow` grant — root, a teammate-`@conductor`, a self-contained `@engineer` — compiles its OWN Dynamic Workflow, and the doctrine that told the two team-lead tiers never to try is retired. Alongside it, the auditor verdict ledger gets code for the first time: a canonical root-owned path, a worktree-divergence check, and the step-to-verdict join that turns a missing audit from invisible into red.**

### Changed

- **`@conductor` and `@engineer` fan out via Dynamic Workflow (#263).** `Workflow` has shipped in both leads' `tools:` frontmatter since #233, but the agent BODIES then told them, repeatedly and in imperative terms, never to use it — `conductor.md` alone carried the instruction five times, including a halt code (`WORKFLOW-SELFCHECK-TOOLSEARCH`) that forbade a conductor from even *looking* for the tool. The grant was therefore dead on arrival at exactly the tier that needed it: it could not self-activate, and the one probe that would have revealed it was prohibited. Worse, the WAVE-COMPLETE contract required reporting `workflow_tool: "absent"` / `fanout: "in-context"` and the wave-review auditor graded those values CORRECT — so the reporting pipeline actively certified the wrong vehicle. The cost was measured on a live `FL03/axiom` sprint: an operator read this doctrine as authority and reversed a CORRECT engineer dispatch four separate times, once instructing the engineer to skip its own `@critic`, which would have failed root's own `shctx plan verify` with `CRITIC-PROOF-MISSING`. Roughly an hour, and six contradictory instructions to a teammate that was working correctly the whole time.

  Inverted across every surface that carried it: `agents/conductor.md` (§Lane walk, §DISPATCH MODE, §Defensive poll, the hotfix ladder, the WAVE-COMPLETE payload, §Halt codes), `agents/engineer.md §Self-contained mode`, `agents/auditor.md §Dispatch-substrate`, `skills/harness/SKILL.md §Workflow tool` + `§Tool presence`, `skills/harness/references/workflow-templates.md`, `skills/shepherd/references/pipeline.md §Lane law` + `§Combo waves` + `§Hotfix ladder`, `wave-routine.md`, `flock.md`, `invariant-matrix.md` rows 3/24/28, and `commands/spawn.md`.

  The distinction the old docs blurred is preserved deliberately: `pipeline.md §Lane law` was RIGHT that a team lead hand-driving its own lane fan-out is correct and never flaggable. Who drives did not change. What was stale is only the "therefore the vehicle is `Agent()`" conclusion. Who drives: the lead. What it drives: a Workflow.

- **`WORKFLOW-VEHICLE-PROBE` replaces the prohibition with a requirement (#263).** A lead reads its own visible tool list for the literal token `Workflow`, once per session, before its first fan-out. Present → compile and dispatch. Genuinely absent → fan out in-context via `Agent()` AND record `fanout_downgrade_reason` — a downgrade is legitimate, a silent one is not. This deliberately does NOT resolve #251 (whether an unavailable `Workflow` presents as denied-at-invocation or invisible-to-discovery, still an open measurement dispute): the probe tests the visible tool list, which is the correct check under either reading, and the downgrade path handles the negative case regardless of which is eventually confirmed. `ToolSearch` remains the WRONG index for the question — `Workflow` is a native top-level primitive, so a nothing-result there means the wrong index was queried, never absence. That platform fact is unchanged; what changed is that the code now names the wrong PROBE (`WORKFLOW-PROBE-WRONG-INDEX`) rather than forbidding the act of probing.

- **The wave-review auditor's grading is inverted with it (#263).** `agents/auditor.md §Dispatch-substrate` graded `workflow_tool: absent` + `fanout: in-context` as EXPECTED and a compiled teammate Workflow as `PRIMITIVE-INVERSION`. Now `workflow_tool: "present"` + `fanout: "workflow"` is the expected shape at every lead tier; an in-context fan-out at a grant-holding tier with a NULL `fanout_downgrade_reason` is `FANOUT-VEHICLE-DOWNGRADE` MEDIUM; and one WITH a reason is an accepted, evidenced downgrade — grade the reason, not the vehicle. The rubric says explicitly not to apply the old reading to an older trace.
