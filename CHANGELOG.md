...
**The dev.5 evidence cluster: the "one dispatcher, waves of dynamic workflows" routine is codified once and driven two ways, its wave-gate facts move out of latent space into three deterministic scripts, and the build wave that shipped it caught two #207-class tool-wiring holes — including root's own missing `Workflow` grant, without which the whole mode was dead on arrival.**

Five issues, all authored 2026-07-17 from the same live `v0.3.8-dev.5` run, form one coherent unit: a single root shepherd driving waves of Dynamic Workflows (no agent-team fanout), proven live, is made canonical — and the tooling and precision that mode needs is built with tests, not prose. The patch was itself produced by a **Dynamic-Workflow sonnet wave gated by the root** (the plugin dogfooding the very routine it codifies); that wave's read-only tool-wiring audit surfaced the two capability gaps below.

### New

- **`/shepherd:start` is resurrected as the codified root-drives-workflows execution mode, defined ONCE and driven two ways (#217).** The proven per-wave routine — `pipeline()` over file-disjoint `shepherd:coder`+`shepherd:auditor` step pairs (redo cap 3, `prescribed_fixes` threaded verbatim), a six-line hard-rule preamble carried into every brief, and a serial five-check root gate (`journal-status` → `loc-count` → cross-step file-disjointness → workspace test gate → append-only MSD ledger + wave commit) — now lives as the single canonical reference `skills/shepherd/references/wave-routine.md`. Two drivers, one routine: **root** runs it directly via the thin, execution-only `commands/start.md` (no `@engineer`/`@critic` planning phase; the plan must already exist), which is ALSO the documented zero-drift fallback when Agent Teams / teammate-conductors are unavailable; a **`@conductor`** runs the identical routine ABBREVIATED and scoped to one lane (`agents/conductor.md §Lane walk`), differing only in scope (one lane vs the sprint) and git-integration authority (deferred to root). `commands/spawn.md` and `agents/shepherd.md` are wired to the same reference so the spawned and direct paths can never drift. `pipeline.md §Gates` gains the wave-return / disk / LOC notes the routine depends on.

- **Three deterministic wave-gate scripts land in `scripts/`, so no sprint counts LOC, detects wave-return, or checks disk in latent space again.**
  - **`scripts/loc-count.py` — the canonical wave-gate LOC counter (#216).** Net production Rust LOC of the working tree vs a base ref: added-minus-removed `.rs` lines OUTSIDE brace-matched `#[cfg(test)]` / `#[cfg(all(test, …))]` / `#[cfg(any(test, …))]` spans, `tests/` directories skipped, untracked new files counted (the counter runs in the root gate BEFORE the wave commit, so a coder's new files are still untracked and `git diff` alone would undercount). The brace matcher is comment- and string/char/raw-string-literal-aware and finds EVERY span in a file — directly fixing the two bugs the issue recorded: the single-span assumption that read a multi-test-module file as 0, and the regex that missed `#[cfg(all(test, …))]`. `#[cfg(not(test))]` is correctly counted as production. `hooks/tests/test_loc_count.sh` pins all of it against a git fixture.
  - **`scripts/journal-status.sh` — the deterministic wave-return signal (#213).** Reads a Dynamic-Workflow run's `journal.jsonl` (the append-only `started`/`result` record stream, keyed by content-hash) and reports `steps / returned / pass / redo / pending` with per-step verdicts; exit 3 (journal absent), 4 (some step pending), 0 (all returned). This is the wave-return TRUTH the harness task registry can't be — it went blind mid-run in the field. Doctrine: the dispatcher records the `runId` + absolute journal path in the plan frontmatter (survives `/compact`) and the background until-loop watchdog on the journal is canonical; task notifications are best-effort (`agents/shepherd.md`, `pipeline.md`). Built and validated against a real journal emitted by this patch's own build wave; `hooks/tests/test_journal_status.sh` pins it.
  - **`scripts/df-guard.sh` — the disk-pressure precheck (#214).** `df-guard.sh --min=12` before ANY cargo invocation (a wave that fills the disk hard-freezes the whole session — even the tool harness stops writing). Portable `df -Pk` parse, exit 0/1/2. The four dev.5-hardened disk rules — df precheck, coder→auditor SHARED lane `CARGO_TARGET_DIR`, delete-on-final-PASS, and root/workspace gate never concurrent with a lane cargo build — are baked into the coder and auditor briefing contracts (`agents/coder.md`, `agents/auditor.md`), `pipeline.md §Gates`, and the wave routine's hard-rule preamble. `hooks/tests/test_df_guard.sh` pins the threshold behavior.

### Fixed

- **`@shepherd` (root) finally GRANTS the `Workflow` tool its own doctrine mandates — the #207 gap one tier up, and a hard blocker for #217 (#217/#207).** Root's `agents/shepherd.md §WORKFLOW SELF-CHECK` mandates compiling Dynamic Workflows, and the whole `/shepherd:start` mode has root drive them directly — but root's `tools:` frontmatter never granted `Workflow`. Root without the grant cannot drive a single wave, so the mode would have been dead on arrival. Fixed at the frontmatter and pinned structurally: `hooks/tests/lint_agent_capabilities.sh` `LEAD_MANDATED_WORKFLOW` now includes `shepherd` alongside `engineer`/`conductor`, and the §WORKFLOW SELF-CHECK prose is reconciled to the post-#207 framing (`present` is the guaranteed path, `absent` is a genuine web/remote runtime denial, not the routine spawn state). Surfaced by the build wave's read-only capability audit.

- **`@critic` finally GRANTS `Bash` — its Step 0.5 deliverable-promise had zero way to run (#207-class, no fallback).** `agents/critic.md §Step 0.5` mandates `shctx deliverable promise` / `shctx audit insert` / `shctx deliverable complete` in bash blocks, but the critic's `tools:` were `Glob, Grep, Read, Skill` — no `Bash`. The critic literally could not register its verdict deliverable; every dispatch left the row `stalled`. The other two read-only reviewers (`auditor`, `discovery`) already carry `Bash` for exactly this. Fixed, and `lint_agent_capabilities.sh` gains a read-only-role Bash-presence block asserting every read-only reviewer whose prose runs `shctx` grants `Bash` (read-only on SOURCE ≠ no-Bash; the mutation bans still hold). Also surfaced by the build wave's audit.

- **The coder's ONE-LOC rule and governance boundary are now verbatim in the profile (#215).** At a redo-cap exit in the field, a coder deleted a mandated mock-double integration test reasoning it "fights the LOC budget" — a false premise (tests/ files and `cfg(test)` bodies are excluded from the count) AND a governance call only the dispatcher may make. `agents/coder.md` gains: (1) the ONE-LOC rule verbatim — every production `*.rs` line counts, `tests/` files and `cfg(test)` bodies do not; (2) a `LOC-BUDGET-GOVERNANCE` halt code — any budget/scope/governance interpretation is a BLOCKED-escalation to the dispatcher, never local adjudication; (3) a hard rule that dropping a mandated deliverable is NEVER a valid LOC remedy.

### Housekeeping

- **`hooks/tests/test_exec_bits.sh` now covers the new `scripts/` tools** (`scripts/*.sh` + `scripts/loc-count.py`), so a path-invoked wave-gate script shipped 100644 fails the suite. **`hooks/tests/test_v638_wiring.sh`** pins every doctrine-wiring leg of all five issues plus the capability fixes (no dangling cross-reference), and all new tests are wired into `hooks/tests/run.sh` (hook suite 68 → 73). **`skills/shepherd/references/invariant-matrix.md`** gains rows 25–27 (the canonical wave routine + root `Workflow` grant; deterministic wave tooling; read-only-role Bash presence).

- **An adversarial verify wave (3 sonnet auditors, gated by the root) hardened the patch before merge — 6 reproduced defects fixed, not shipped.** `loc-count.py`: a wholly-deleted `.rs` file now counts its removed lines (was silently 0 — the CRITICAL), a renamed file resolves its base blob at the OLD path (removed test-span lines were miscounted as production), and `#[cfg(test)] // comment` is recognized. `journal-status.sh` tolerates a valid-JSON-but-non-object line instead of crashing. `df-guard.sh` rejects a non-numeric `--min` with a distinct exit 2 (was an unbound-variable crash exiting 1, colliding with INSUFFICIENT). **Doctrine reconciliation:** two auditors independently caught that the root-drives-workflows mode contradicted the standing Dispatch law, so `skills/shepherd/SKILL.md` and `agents/shepherd.md` prohibitions #2/#12 + the side-effect boundary now carve out root dispatching `@coder` / committing source ONLY in this mode (where root runs the same wave gate a conductor would). **Mechanical follow-through:** the `@critic` Bash grant is matched by a `bash_guard.sh` Check 3 read-only-mutate guard so a critic cannot mutate source/filesystem via shell (shctx still passes), pinned by the new `hooks/tests/test_readonly_bash_guard.sh` — which also backfills the previously-untested `@discovery` leg. Every fix ships with the regression test that catches it.
