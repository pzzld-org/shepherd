...
- **`hooks/tests/test_exec_bits.sh` now covers the new `scripts/` tools** (`scripts/*.sh` + `scripts/loc-count.py`), so a path-invoked wave-gate script shipped 100644 fails the suite. **`hooks/tests/test_v638_wiring.sh`** pins every doctrine-wiring leg of all five issues plus the two capability fixes (no dangling cross-reference), and all four new tests are wired into `hooks/tests/run.sh` (hook suite 68 → 72). **`skills/shepherd/references/invariant-matrix.md`** gains rows 25–27 (the canonical wave routine + root `Workflow` grant; deterministic wave tooling; read-only-role Bash presence).
