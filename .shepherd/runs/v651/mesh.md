...
| R63a | **This seed is the #319 experiment and its result is a data point.** `shepherd seed verify .shepherd/runs/v651/seed.md` -> `warn  footprint 377 lines > smell threshold 300` / `OK: 0 hard failures, 1 warning(s)`, exit 0. A 13-deliverable repair sprint authored against the gate read from source lands at 377 of the 400-line `sprint-seed` cap — 94% consumed, and over the smell threshold. The cap is not merely mis-tiered for `patch-seed` (R43); it is tight for `sprint-seed` too. Warnings are non-blocking by design (`crates/cli/src/cmd/wave_b2_seed.rs` USAGE at `:15` -> "Exit 1 on >=1 HARD failure … 0 otherwise (warnings allowed)"), so the seed is committed as authored rather than compressed to game the threshold. |
