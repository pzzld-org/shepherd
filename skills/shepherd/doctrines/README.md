...
| `engineer-self-contained-plan.md` | (v6.2.5 #169) Plan construction is the whole point of the engineer; a self-contained teammate runs its own in-session `@discovery` + embedded critic pass + ≥1 revision and returns a hash-tied **critic-proof** (`shctx plan record-critique`/`verify`); root accepts via a thin gate, no re-critique |
