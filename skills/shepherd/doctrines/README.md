...
| `coordinate-active-drive.md` | (v6.0.5, #113/#98/#112) The dispatch→coordinate transition is active, never a passive wait — root runs wake→act→probe→yield-to-events after `TeamCreate` and ends its turn only at enumerated operator-pauses; teammates begin on boot; mechanically backstopped by the `coordinate_drive_guard.sh` Stop hook |
...
| `senior-engineering.md` | (v6.1.6) The senior-engineering operating standard for `@auditor` + `@coder` — eight primitives (comprehend intent / Chesterton's fence, root-cause over symptom, blast-radius-weighted severity, justify the tradeoff, conform to THIS project & user by precedence ladder, cross-concern systemic-risk detection, bounded restraint, preserved read-only/tier discipline). Always-on (cited in both profiles, `[SENIOR-STANDARD]` brief block) + the operative contract `/shepherd:ponytail` invokes. Builds on existing doctrines; `SENIOR-STANDARD-MISUSE` anti-pattern |
