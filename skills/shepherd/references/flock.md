...
**Files-as-context-bus.** A lane brief carries the lane-plan PATH — `{run_dir}/lanes/{lane}/plan.md`, conductor-owned (`agents/conductor.md`) — never a pasted plan slice; the dispatched role reads the file. Symmetrically, @auditor / @discovery / @worker write their full output to a file under `{run_dir}/lanes/{lane}/reports/` and return ≤15 lines to the dispatcher (verdict + pointer) — the file is the payload, the return is the receipt. **@coder is the one exception**: its deliverable is the diff plus a schema-validated STRUCTURED RESULT, never a file under `reports/` (`agents/coder.md §Output discipline`, §@coder below) — a coder that writes one is producing an artifact nothing downstream reads.
...
Sonnet, one per step (a single non-overlapping `[FILE-SCOPE]`). Zero-overlap coders MUST be dispatched in the SAME message; sequential dispatch of parallel-safe coders is a process violation. Sequential is REQUIRED for schema migrations (single writer), rename cascades across public re-exports, or a critic-flagged dependency. Progress bar: ≥ 50 LOC per coder; < 30 LOC merge with adjacent; > 8 files split. Coders run ZERO build/compile/lint AND ZERO git — write files, return them in the STRUCTURED RESULT's `files_touched`, never a report file, never commit (read-only `git status`/`diff`/`log`/`show`/`rev-parse` only; `coder_git_guard.sh` enforces it, `CODER-GIT-WRITE`). A coder's own account carries NO verification authority; `git diff` against live HEAD is what a wave-review PASS is actually graded against (`agents/coder.md §Output discipline`).

**`files_touched` routing.** The wave dispatches coders through the `Workflow` tool's `agent()` call with `schema: CODER_RESULT` set — that return comes back to the SCRIPT that issued it, never routed away to a side session (unlike a bare `Agent()` dispatch — GH #270). The wave's own return value aggregates every step's `files_touched` and carries it back to whichever lead invoked the workflow — root, a teammate-conductor, or a self-contained engineer. This holds whether the script is file-referenced (`workflows/wave.js`, retained as the worked reference skeleton) or authored inline per lane — the schema+return contract lives in the `agent()` primitive itself, not in how the script was loaded. It does NOT hold for a lead that hand-rolls coder dispatch via the bare `Agent()` tool instead of `Workflow`: that path has no return channel at all, so `files_touched` is not expressible and pathspec-explicit staging cannot be sourced from it — which is exactly why §Cascade forbids hand-rolling fan-out in-context (#263); the gap is a reason to run the sanctioned vehicle, never a reason to fall back to `-A`. The conductor runs the gate sequence once per wave after all rebases, and stages+commits each coder's returned `files_touched` paths only after the wave-review returns PASS — a REDO re-runs the named coder over the SAME uncommitted files, nothing to unwind (`skills/shepherd/references/pipeline.md §Gates` + §Wave review + REDO).

**Cross-lane dependencies** — pause-for-dependency is retired: a needed symbol that is missing, outside `[FILE-SCOPE]`, and not owned by a wave-sibling is a graph edge the engineer composes (the compiled segment await-orders it); the coder does NOT pause. In-sprint-but-unsequenced → leave the WIP files in the worktree, note them in the structured result's `files_touched` + `assumptions`, file a `BRIEF-AMENDMENT REQUEST` (never a self-commit). Out-of-scope → finish assigned scope, file a finding at close. NEVER expand scope silently or add a TODO; scope overlap → `SCOPE OVERFLOW`.
...
**INSIGHTS** — a coder MAY append an optional `## INSIGHTS` block (kinds `relocation`/`extension`/`duplication`/`consolidation`/`gap`/`nit`) for the next engineer; taxonomy + template + capture hook are canonical in `skills/adaptation/SKILL.md §INSIGHTS`. Best-effort only — a schema-constrained dispatch does not collect chat prose, only the STRUCTURED RESULT, so a finding that MUST surface belongs in `assumptions` instead. NEVER use it for scope changes (file a `BRIEF-AMENDMENT REQUEST`) or style venting. A project's `.claude/doctrines/*.md`, injected into the brief preamble, WIN over framework guidance on conflict. A dedup-grep hit is REUSE-if-it-fits, else JUSTIFY-NEW naming the missing invariant. Body: `agents/coder.md`.
