...
One primitive per axis, never inverted — the fan-out axis stops being DRIVER-CONDITIONAL (`skills/harness/SKILL.md §Workflow tool`, #263, CC 2.1.212): lanes = **Agent Teams**; one step = one **subagent**; a wave's gate-free step fan-out = a compiled **Dynamic Workflow** over subagents AT EVERY LEAD TIER holding the grant — root, a teammate-`@conductor`, or a self-contained `@engineer`. `Workflow` ships in the `tools:` frontmatter of `@conductor` and `@engineer` (#233) and that grant is now LIVE: the 6.3.9-era "hard-denied inside a subagent" reading is RETIRED as the standing instruction (#263). CRITICAL NUANCE that survives the inversion untouched: a team lead hand-driving its OWN lane fan-out is CORRECT and never flaggable — who drives the fan-out did not change. What was stale is only the "therefore the vehicle is `Agent()`" conclusion; the vehicle a lead drives is now a compiled Dynamic Workflow, not a hand-rolled batch of `Agent()` calls (`skills/shepherd/references/wave-routine.md §Per-wave compile`). In-context `Agent()` fan-out becomes the DOWNGRADE path at any grant-holding tier — legitimate only after a `WORKFLOW-VEHICLE-PROBE` (`skills/shepherd/references/wave-routine.md`) finds `Workflow` genuinely absent from the visible tool list, and recorded with a `fanout_downgrade_reason`; a SILENT downgrade is a wave-review finding (`FANOUT-VEHICLE-DOWNGRADE`), never a certified-correct outcome. Every passage below that newly authorizes a conductor to compile a Workflow is bound by `skills/shepherd/SKILL.md §Fan-out counterweight` (#256): file-disjointness authorizes concurrent WRITES, not concurrent BUILDS — fan out fixes, verify once centrally; the `[coder].max_parallel_lanes` cap and the platform's ~16 concurrent-agent cap both still bind inside a Workflow. Two constructions are hook-refused:

> Halt: `PRIMITIVE-INVERSION — workflow-spawns-teammates`. Refuse the workflow-spawn. (Root tier only.)

> Halt: `PRIMITIVE-INVERSION — handrolled-fanout` (#263). A gate-free step fan-out compiles to a Dynamic Workflow at whichever lead tier holds the grant — root, teammate-conductor, or self-contained engineer; hand-rolling that fan-out in-context via individual `Agent()` calls is this violation WHEREVER THE GRANT IS HELD, unless a `WORKFLOW-VEHICLE-PROBE` found `Workflow` genuinely absent from the visible tool list and the dispatcher recorded a `fanout_downgrade_reason` — that recorded downgrade is not this violation, it is the required fallback. A team lead hand-driving its OWN lane fan-out is still CORRECT and never flaggable; only the vehicle it drives changed.
...
A combo wave dispatches mixed read-only lanes as ONE parallel fan-out — a compiled Dynamic Workflow (`skills/harness/SKILL.md §Workflow tool`), one vehicle regardless of which lead dispatches it: root, or a teammate (e.g. the self-contained engineer's own discovery wave, §INTRO below) — `Workflow` ships in `@conductor`'s and `@engineer`'s `tools:` frontmatter and that grant is LIVE, so the split-by-dispatcher reading collapses (#263). In-context `Agent()` dispatch is the DOWNGRADE path only, taken on a confirmed `WORKFLOW-VEHICLE-PROBE` absence and recorded with a `fanout_downgrade_reason` (`skills/shepherd/references/wave-routine.md`); a dispatcher newly compiling a combo-wave Workflow is bound by `skills/shepherd/SKILL.md §Fan-out counterweight` (#256). Variants:
