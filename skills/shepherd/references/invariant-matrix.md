...
| 3 | A gate-free step fan-out **compiles to a Dynamic Workflow at EVERY lead tier holding the grant** — root, teammate-`@conductor`, self-contained `@engineer` alike; a lead drives its OWN fan-out and the vehicle is the Workflow, with in-context `Agent()` reserved for a probed, recorded downgrade (#263, supersedes the #220 root-only partition) | `dispatch_guard.sh` Check 6 (flag, every lead tier)+`skills/shepherd/references/pipeline.md §Lane law` (driver-agnostic); `test_v643_wiring.sh` pins the vehicle | flag+doctrine+test | **live (doctrine)** |
...
| 28 | The fan-out vehicle is NOT driver-conditional: every lead tier holding the grant compiles a Dynamic Workflow, and in-context `Agent()` is the DOWNGRADE path — taken only when `WORKFLOW-VEHICLE-PROBE` finds the literal token absent from the visible tool list, and always recorded with a `fanout_downgrade_reason` (a silent downgrade is `FANOUT-VEHICLE-DOWNGRADE`). The probe is a REQUIREMENT, never a prohibition; `ToolSearch` is the wrong index for it (`WORKFLOW-PROBE-WRONG-INDEX`, retires `WORKFLOW-SELFCHECK-TOOLSEARCH`) | platform fact canonical in `skills/harness/SKILL.md §Workflow tool`/`§Tool presence`; `conductor.md §DISPATCH MODE` + `engineer.md §Self-contained mode` + `wave-routine.md`/`pipeline.md`/`workflow-templates.md` driver-AGNOSTIC; `test_v643_wiring.sh` (#263, supersedes `test_v639_wiring.sh`'s #220 block) | doctrine+test | **live** |
