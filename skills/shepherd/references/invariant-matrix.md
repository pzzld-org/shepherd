...
| 3 | A gate-free step fan-out **compiles to a Dynamic Workflow on a LIVE Agent-Teams teammate substrate** — root, teammate-`@conductor`, self-contained `@engineer` alike; on an **Agent-tool subagent substrate** the identical fan-out dispatches in-context via `Agent()` instead, correct and unflagged there. A lead drives its OWN fan-out on either substrate; only the vehicle tracks the substrate, never the tier (#263, supersedes the #220 "any spawned role" over-generalization) | `dispatch_guard.sh` Check 6 (flag, every substrate)+`skills/shepherd/references/pipeline.md §Lane law` (substrate-conditional); `test_v643_wiring.sh` pins the vehicle | flag+doctrine+test | **live (doctrine)** |
...
| 24 | ALL THREE leads (`shepherd`, `@engineer`, `@conductor`) GRANT `Workflow` in `tools:` frontmatter; whether the grant is actually LIVE is a fact about SUBSTRATE, never tier — root's substrate is never in question (always the top-level session, never a subagent), so root's copy is always live; a teammate-`@engineer`/`@conductor` on a LIVE Agent-Teams teammate substrate has it live too and drives its OWN fan-out via a compiled Dynamic Workflow; the same role wearing a teammate's brief as an Agent-tool subagent (substrate never went live at spawn) drives it via in-context `Agent()` instead — correct and unflagged there (supersedes the v6.3.9/#220 "teammate leads MUST NOT carry the inert grant" partition, which made the grant unreachable at exactly the tier that needed it) | `lint_agent_capabilities.sh` (`LEAD_MANDATED_WORKFLOW="shepherd engineer conductor"`) + `hooks/tests/test_lead_workflow_tool.sh` (stripping `Workflow` from ANY lead fails the lint); `auditor.md §Dispatch-substrate` grades `workflow_tool: "present"` + `fanout: "workflow"` as correct on a live Agent-Teams teammate substrate, and `workflow_tool: "absent"` + `fanout: "in-context"` with a recorded `fanout_downgrade_reason` as equally correct on an Agent-tool subagent substrate (#263, was #220/#207) | lint+test+auditor | **live** |
...
| 26 | Wave-gate facts are computed in deterministic space, never latent: production LOC by `${CLAUDE_PLUGIN_ROOT}/scripts/loc-count.py` (brace-matched `cfg(test)` exclusion — #216), wave-return by polling `${CLAUDE_PLUGIN_ROOT}/scripts/journal-status.sh` on the run's `journal.jsonl` not the blind task registry (#213), disk headroom by `${CLAUDE_PLUGIN_ROOT}/scripts/df-guard.sh --min=12` before any cargo (#214) | `hooks/tests/test_loc_count.sh` / `test_journal_status.sh` / `test_df_guard.sh` pin behavior; `test_exec_bits.sh` pins the exec bit; `pipeline.md §Gates` + `wave-routine.md` cite them | test+doctrine | **live (tested)** |
...
| 28 | The fan-out vehicle is SUBSTRATE-conditional, never driver-conditional: on a LIVE Agent-Teams teammate substrate the lead compiles a Dynamic Workflow; on an Agent-tool subagent substrate — including a "teammate" brief whose substrate never went live at spawn — the identical fan-out dispatches in-context via `Agent()`, correct and the ONLY option there, not a downgrade. In-context `Agent()` is the DOWNGRADE path ONLY on a live teammate substrate — taken only when `WORKFLOW-VEHICLE-PROBE` finds the literal token absent from the visible tool list, and always recorded with a `fanout_downgrade_reason` (a silent downgrade on a live teammate substrate is `FANOUT-VEHICLE-DOWNGRADE`). The probe is a REQUIREMENT, never a prohibition, and confirms SUBSTRATE, never a dormant grant; `ToolSearch` stays the wrong index for it — a `ToolSearch select:Workflow` null is a false negative by construction (`Workflow` is native, never a deferred `ToolSearch` target) and establishes nothing, neither presence nor absence (`WORKFLOW-SELFCHECK-TOOLSEARCH`) | platform fact canonical in `skills/harness/SKILL.md §Workflow tool`/`§Tool presence`; `conductor.md §DISPATCH MODE` + `engineer.md §Self-contained mode` + `wave-routine.md`/`pipeline.md`/`workflow-templates.md` SUBSTRATE-conditioned; `test_v643_wiring.sh` (#263, supersedes `test_v639_wiring.sh`'s #220 block) | doctrine+test | **live** |
| 34 | A dispatcher NEVER treats an unarrived completion notification as evidence that dispatched work is unfinished. On the in-context `Agent()` path the notification does not arrive AT ALL (measured 3/3, two lanes, three coders — #270), so the poll IS the signal, not a backstop: worktree state (`git status --porcelain` / `diff --shortstat`) is ground truth for a coder, ahead of the task registry, and `SendMessage`'s `had no active task; resumed from transcript` reply is a confirmed completion signal. Blocking on the notification stalls a lane indistinguishably from a slow coder while its heartbeat still reads healthy | `agents/conductor.md §Defensive poll` (#224 + #270), `wave-routine.md` serial gate step 1, `hooks/tests/test_v644_wiring.sh` | doctrine+test | **live (tested)** |
