...
| 24 | `@engineer`/`@conductor` GRANT the `Workflow` tool their fan-out doctrine mandates (§WORKFLOW SELF-CHECK: Dynamic-Workflow fan-out is the default under `[spawn].lead_effort=ultracode`) — a lead cannot silently lose it again and fall back to the slow in-context `Agent()` path | `lint_agent_capabilities.sh` `LEAD_MANDATED_WORKFLOW` block + `hooks/tests/test_lead_workflow_tool.sh` (strip-and-reintroduce proof, both leads independently); runtime layer agrees via `auditor.md §Dispatch-substrate discipline` (unexplained `absent` → regression signal) (#207) | lint+test+auditor | **live** |
