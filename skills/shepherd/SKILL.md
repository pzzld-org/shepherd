...
| `doctrines/capability-discovery.md` | Session start; "is plugin/skill X available?" | **NEW v6.2.0 (#146) — auto-adapt to the environment**: a SessionStart probe enumerates installed plugins/skills and writes an EPHEMERAL capability roster (`<ns>/cache/discovered-capabilities.json`, distinct from the curated `toolkit.json`), merged into the `[TOOLKIT]` surfaces labeled auto-discovered; guarded-integration pattern (if `/remember` available → handoff/CLOSE-FINALIZE + resume, else native; `superpowers`; `pr-review-toolkit`); records native `Workflow`-tool presence (degrade to `Agent(...)` when absent); config `[discovery].auto_capabilities`; zero hot-path cost, never hard-depends, fail-open. |
...
| `doctrines/autonomous-sentinel.md` | Seed with `close: autonomous-sentinel`; any supervised self-heal soak | **NEW v6.2.0 (#148) — authorized supervised self-heal**: the supervised-remediation SUPERSET of SOAK-LOOP. PROBE → CLASSIFY (HOLD/REGRESSED/NEW) → ACT (≤S `@coder` hotfix via the hotfix-dispatch ladder → gates-before-deploy → re-probe) → TERMINATE (K clean ticks / N-HF cap / hard-stop). NEVER fires by default — gated behind `[close].autonomous_sentinel = "on"` + a `close: autonomous-sentinel` seed declaration + a complete `sentinel_rails` block. Detection-only SOAK-LOOP stays the default. |
