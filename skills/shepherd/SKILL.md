...
**Probe once per session, before the FIRST fan-out (`WORKFLOW-VEHICLE-PROBE`).** Read your own visible tool list for the literal token `Workflow` — this is a REQUIREMENT, not a prohibition, since a lead that never probes cannot know which vehicle it owns. Present → compile and dispatch a Dynamic Workflow, the default and expected outcome at every lead tier. Genuinely absent (the token is not in the visible tool list) → fan out in-context via `Agent()`, the whole `parallel_with` clique in ONE message, and record BOTH `fanout: "in-context"` AND a non-null `fanout_downgrade_reason` in the WAVE-COMPLETE payload; a downgrade with no recorded reason at a grant-holding tier is `FANOUT-VEHICLE-DOWNGRADE` — a wave-review finding, not a certified-correct outcome. NEVER `ToolSearch` for `Workflow` to answer this question (`WORKFLOW-PROBE-WRONG-INDEX`): `ToolSearch` resolves DEFERRED tools only, `Workflow` is a native top-level primitive, so a nothing-result means the wrong index was queried, NEVER absence (`skills/harness/SKILL.md §Tool presence`). The visible tool list is the only valid oracle. This code REPLACES `WORKFLOW-SELFCHECK-TOOLSEARCH` (`references/escalation.md §Halt-code index`): what is forbidden is the WRONG PROBE, never the act of probing.
