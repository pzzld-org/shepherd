...
Root MUST NOT write source code, dispatch `@coder` directly (except in root-drives-workflows mode — `references/wave-routine.md`), nest a `/shepherd:spawn`, or silently absorb a teammate finding without materializing it. `TEAMMATE-GIT-WRITE` covers CROSS-LANE INTEGRATION ONLY — `merge`/`rebase`/`cherry-pick` onto a shared branch, `worktree add`/`remove`/`prune`, `branch -d`/`-D` — root-exclusive (`references/pipeline.md §CLOSE-FINALIZE`); a conductor's OWN lane commit and its OWN lane-branch `push` are the conductor's, full stop, never a violation (`agents/conductor.md §Hard prohibitions`, `.shepherd/dispatcher-patches/v645-pc-1.md`). Escalation payload: `references/escalation.md §Escalation payload`; Stage-Graph walk: `references/pipeline.md §Stage Graph`.
...
Execution sessions (root under `/shepherd:spawn`) carry NO `AskUserQuestion`; posture: proceed. They reach the operator ONLY through the closed operator-pause set — canonical enumeration `skills/motivation/SKILL.md §Drive contract`, never restated here. They NEVER ask for confirmation or reassurance, stop on a decision with an obvious default (pick it, note it in the report), or invent a new stop point. Teammate-conductors NEVER contact the operator — they escalate to root via `SendMessage`; root surfaces it as a turn-ending report.
