...
- `shctx profile <list|show|enable|disable|sync>` — TOML profiles in `.artifacts/profiles/` ↔ `profiles_defs`.
- `shctx mem <add|search|list|pin|unpin>` — project memories (replaces external `remember` plugin).
- `shctx lock <show|acquire|release|reap>` — file lock at `.artifacts/shepherd.lock`.
- `shctx lint` — naming-convention check over `.artifacts/`.
...
- `--scope=artifacts` — re-hash files in `.artifacts/` matching configured kinds. Updates `artifacts`.
...
`.artifacts/` paths follow strict patterns; `shctx lint` enforces them. See `references/naming-conventions.md`.
...
The plugin's `init` writes `.artifacts/CONVENTIONS.md` — the same table, mirrored into the consumer project for quick local reference.
...
- `examples/journal-entry.md` — sample `.artifacts/docs/journal/YYYY-MM-DD.md` layout.
