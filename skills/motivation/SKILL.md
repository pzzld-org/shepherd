...
**Two kinds of stop.** Operator-pause is a legitimate, closed set of 7 (pre-spawn approval gate; `HARD-STOP`; operator-question escalation; `CROSS-TEAMMATE-DISPUTE` adjudication; scope-confirmation `confirm minor`/`confirm version`; end-of-scope ROOT CLOSE REPORT; explicit `OPERATOR-INTERRUPT`) — each emits a concrete, actionable question. Passive-wait (ending the turn at the dispatch boundary, post-WAVE-COMPLETE, or mid-wave, with no question pending) is forbidden. Yield to events, never the operator, unless only the operator can answer.
