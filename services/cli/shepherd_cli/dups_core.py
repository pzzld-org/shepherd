#!/usr/bin/env python3
"""dups_core — field-shape similar-struct engine for `shepherd dups` (v6.1.8, #157).

Pure stdlib. The relocated, package-resident home of the engine that lived at
``skills/context/scripts/dups-core.py`` (byte-parity: the code below is that
script's, unchanged — only this header moved). Invoked by
:mod:`shepherd_cli.commands.dups` as a subprocess of THIS interpreter
(``[sys.executable, "-m", "shepherd_cli.dups_core", ...]``) with the SAME argv
shape ``cmd_dups.sh`` used to build for ``python3 dups-core.py``; never run
directly by operators. ``main(argv)`` is equally importable for in-process
callers/tests. The argparse ``prog`` stays ``"dups-core.py"`` so usage/error
text (e.g. a malformed ``--fail-on`` choice, exit 2) remains byte-identical
to the bash-era engine.
