...
`--json` prints **only** the verdict object — that is the contract `shctx eval`
...
`shctx eval` is the shepherd-side glue: it resolves a subject from the registry
(e.g. the reflection note for a sprint), calls this service, and records the
verdict into `eval_runs` (migration `0018`), which the dash surfaces. See
`skills/context/scripts/cmd_eval.sh` and `docs/configuration.md §[eval]`.
