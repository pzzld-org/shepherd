#!/usr/bin/env bash
# hooks/tests/test_workflow_model_guard.sh — tests for workflow_model_guard.sh (#178)
#
# Covers the PreToolUse(Workflow) dispatch-model-pin guard:
#   1.  bare `agent(prompt)` (no opts arg)             → DENY + WORKFLOW-MODEL-PIN-MISSING
#   2.  `agent(prompt, {model: "sonnet"})`              → PASS
#   3.  `agent(prompt, {agentType: "shepherd:coder"})`  → PASS
#   4.  opts object present, neither key                → DENY
...
if is_deny "$out" && printf '%s' "$out" | grep -q 'neither model:'; then
  pass "4. opts present, missing both keys: DENY"
else
  fail "4. opts present, missing both keys: DENY" "out=${out:0:160}"
fi

echo "== workflow_model_guard.sh — pinned calls PASS =="

total=$((total+1))
out=$(run_hook "$(P 'const r = await agent("do the thing", {model: "sonnet"})' "$tmp")")
if ! is_deny "$out" && ! is_context "$out"; then
  pass "2. model: \"sonnet\" pinned: PASS (silent)"
else
  fail "2. model: \"sonnet\" pinned: PASS (silent)" "unexpected output: ${out:0:160}"
fi

total=$((total+1))
out=$(run_hook "$(P 'const r = await agent("do the thing", {agentType: "shepherd:coder"})' "$tmp")")
if ! is_deny "$out" && ! is_context "$out"; then
  pass "3. agentType: \"shepherd:coder\": PASS (silent)"
else
  fail "3. agentType: \"shepherd:coder\": PASS (silent)" "unexpected output: ${out:0:160}"
fi
...
total=$((total+1))
out=$(run_hook "$(P 'const a = await agent("compliant", {model: "sonnet"})
const b = await agent("also compliant", {agentType: "shepherd:auditor"})
const c = await agent("the culprit", {schema: X})' "$tmp")")
if is_deny "$out" \
   && printf '%s' "$out" | grep -q '1 agent() call(s)' \
   && printf '%s' "$out" | grep -q 'the culprit'; then
...
out=$(run_hook "$(P 'const results = await pipeline(
  ...
  review => parallel(review.findings.map(f => () =>
    agent(`Adversarially verify: ${f.title}`, {label: `verify:${f.file}`, phase: "Verify", model: "sonnet"})
  ))
)' "$tmp")")
if is_deny "$out" \
   && printf '%s' "$out" | grep -q '1 agent() call(s)' \
