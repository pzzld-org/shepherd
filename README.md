...
The v6.5.0 component is published as `fl03:shepherd@6.5.0`. Its WIT contract,
...
```sh
curl --fail --location \
  https://raw.githubusercontent.com/FL03/shepherd/v6.5.0/scripts/install-shepherd.sh \
  --output /tmp/install-shepherd.sh
SHEPHERD_VERSION=6.5.0 bash /tmp/install-shepherd.sh
```
...
Invoke-WebRequest `
  https://raw.githubusercontent.com/FL03/shepherd/v6.5.0/scripts/install-shepherd.ps1 `
  -OutFile $installer
$env:SHEPHERD_VERSION = '6.5.0'
& $installer
```
...
```sh
codex plugin marketplace add FL03/shepherd --ref v6.5.0
