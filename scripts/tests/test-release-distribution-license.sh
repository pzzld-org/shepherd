...
for target in aarch64-apple-darwin aarch64-unknown-linux-gnu x86_64-apple-darwin x86_64-unknown-linux-gnu; do
  for name in "shepherd-6.5.2-${target}.tar.gz" "shepherd-${target}.tar.gz"; do
    tar -C "$payload" -czf "$assets/$name" LICENSE THIRD_PARTY_NOTICES.md THIRD_PARTY_LICENSES shepherd
  done
done
for name in shepherd-6.5.2-x86_64-pc-windows-msvc.zip shepherd-x86_64-pc-windows-msvc.zip; do
  (cd "$payload" && zip -qr "$assets/$name" LICENSE THIRD_PARTY_NOTICES.md THIRD_PARTY_LICENSES shepherd.exe)
done
for name in shepherd-component-6.5.2-wasm32-wasip2.tar.gz shepherd-component-wasm32-wasip2.tar.gz; do
...
package_tarballs=$(release_package_names 6.5.2)
while IFS= read -r tarball; do
  tar -C "$payload" -czf "$assets/$tarball" package
done <<<"$package_tarballs"
scripts/verify-release-distribution.sh "$assets" 6.5.2
...
tar -C "$tampered_payload" -czf "$tmp_dir/tampered/shepherd-component-6.5.2-wasm32-wasip2.tar.gz" \
  LICENSE THIRD_PARTY_NOTICES.md THIRD_PARTY_LICENSES shepherd-component.wasm
if scripts/verify-release-distribution.sh "$tmp_dir/tampered" 6.5.2 >/dev/null 2>&1; then
