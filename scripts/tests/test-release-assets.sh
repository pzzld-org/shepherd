...
for target in "${targets[@]}"; do
  printf 'versioned %s\n' "$target" > "$asset_dir/shepherd-6.4.8-$target"
  printf 'stable %s\n' "$target" > "$asset_dir/shepherd-$target"
done
for name in \
  shepherd-component-6.4.8-wasm32-wasip2.tar.gz \
  shepherd-component-wasm32-wasip2.tar.gz \
  pzzld-component-runtime-6.4.8.tgz \
  pzzld-pi-claude-6.4.8.tgz \
  pzzld-pi-codex-6.4.8.tgz \
  pzzld-pi-shepherd-6.4.8.tgz; do
  printf 'fixture %s\n' "$name" > "$asset_dir/$name"
done
...
"$verifier" "$asset_dir" "$asset_list" 6.4.8
[[ $(wc -l < "$asset_list" | tr -d ' ') == 32 ]] || fail 'expected 32 publishable files'
rg -Fxq "$asset_dir/shepherd-6.4.8-aarch64-apple-darwin.tar.gz" "$asset_list"
rg -Fxq "$asset_dir/shepherd-6.4.8-aarch64-apple-darwin.tar.gz.sha256" "$asset_list"

cp -R "$asset_dir" "$tmp_dir/missing"
find "$tmp_dir/missing" -name '*.sha256' -exec rm -f {} +
if "$verifier" "$tmp_dir/missing" "$tmp_dir/missing.txt" 6.4.8 >/dev/null 2>&1; then
  fail 'missing checksum sidecar must fail'
fi

cp -R "$asset_dir" "$tmp_dir/wrong-name"
mv "$tmp_dir/wrong-name/pzzld-pi-shepherd-6.4.8.tgz" \
  "$tmp_dir/wrong-name/pzzld-pi-shepherd-6.4.9.tgz"
mv "$tmp_dir/wrong-name/pzzld-pi-shepherd-6.4.8.tgz.sha256" \
  "$tmp_dir/wrong-name/pzzld-pi-shepherd-6.4.9.tgz.sha256"
if "$verifier" "$tmp_dir/wrong-name" "$tmp_dir/wrong-name.txt" 6.4.8 >/dev/null 2>&1; then
...
if "$verifier" "$tmp_dir/tampered" "$tmp_dir/tampered.txt" 6.4.8 >/dev/null 2>&1; then
  fail 'checksum mismatch must fail'
fi

if "$verifier" "$asset_dir" "$asset_dir/release-files.txt" 6.4.8 >/dev/null 2>&1; then
