...
for target in "${targets[@]}"; do
  printf 'versioned %s\n' "$target" > "$asset_dir/shepherd-6.5.1-$target"
  printf 'stable %s\n' "$target" > "$asset_dir/shepherd-$target"
done
for name in \
  shepherd-component-6.5.1-wasm32-wasip2.tar.gz \
  shepherd-component-wasm32-wasip2.tar.gz \
  pzzld-component-runtime-6.5.1.tgz \
  pzzld-pi-claude-6.5.1.tgz \
  pzzld-pi-codex-6.5.1.tgz \
  pzzld-pi-shepherd-6.5.1.tgz; do
  printf 'fixture %s\n' "$name" > "$asset_dir/$name"
done
...
if "$verifier" "$tmp_dir/missing" "$tmp_dir/missing.txt" 6.5.1 >/dev/null 2>&1; then
  fail 'missing checksum sidecar must fail'
fi

cp -R "$asset_dir" "$tmp_dir/wrong-name"
mv "$tmp_dir/wrong-name/pzzld-pi-shepherd-6.5.1.tgz" \
  "$tmp_dir/wrong-name/pzzld-pi-shepherd-6.5.2.tgz"
mv "$tmp_dir/wrong-name/pzzld-pi-shepherd-6.5.1.tgz.sha256" \
  "$tmp_dir/wrong-name/pzzld-pi-shepherd-6.5.2.tgz.sha256"
if "$verifier" "$tmp_dir/wrong-name" "$tmp_dir/wrong-name.txt" 6.5.1 >/dev/null 2>&1; then
...
if "$verifier" "$tmp_dir/tampered" "$tmp_dir/tampered.txt" 6.5.1 >/dev/null 2>&1; then
  fail 'checksum mismatch must fail'
fi

if "$verifier" "$asset_dir" "$asset_dir/release-files.txt" 6.5.1 >/dev/null 2>&1; then
