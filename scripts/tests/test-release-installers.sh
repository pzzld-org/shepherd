...
asset_for() {
  ...
  local expected="$3"
  local actual
  actual=$(SHEPHERD_OS="$os" SHEPHERD_ARCH="$arch" SHEPHERD_VERSION=6.4.9 \
    "$installer" --print-asset)
  expect_eq "$actual" "$expected" "asset for $os/$arch"
}

asset_for Darwin arm64 shepherd-6.4.9-aarch64-apple-darwin.tar.gz
asset_for Darwin x86_64 shepherd-6.4.9-x86_64-apple-darwin.tar.gz
asset_for Linux aarch64 shepherd-6.4.9-aarch64-unknown-linux-gnu.tar.gz
asset_for Linux x86_64 shepherd-6.4.9-x86_64-unknown-linux-gnu.tar.gz
asset_for MINGW64_NT x86_64 shepherd-6.4.9-x86_64-pc-windows-msvc.zip

if SHEPHERD_OS=MINGW64_NT SHEPHERD_ARCH=arm64 SHEPHERD_VERSION=6.4.9 \
  "$installer" --print-asset >/dev/null 2>&1; then
  fail 'Windows ARM64 must fail until the release matrix publishes that asset'
fi
if SHEPHERD_OS=Linux SHEPHERD_LIBC=musl SHEPHERD_ARCH=x86_64 SHEPHERD_VERSION=6.4.9 \
  "$installer" --print-asset >/dev/null 2>&1; then
  fail 'musl Linux must fail until the release matrix publishes that asset'
fi

explicit_url=$(SHEPHERD_OS=Linux SHEPHERD_ARCH=x86_64 SHEPHERD_VERSION=6.4.9 \
  "$installer" --print-url)
expect_eq "$explicit_url" \
  'https://github.com/FL03/shepherd/releases/download/v6.4.9/shepherd-6.4.9-x86_64-unknown-linux-gnu.tar.gz' \
...
asset_dir="$release_root/download/v6.4.9"
mkdir -p "$asset_dir" "$tmp_dir/payload"
payload="$tmp_dir/payload/shepherd"
cat >"$payload" <<'EOF'
#!/usr/bin/env sh
printf 'shepherd fixture 6.4.9\n'
EOF
chmod 755 "$payload"
...
asset='shepherd-6.4.9-x86_64-unknown-linux-gnu.tar.gz'
(
  cd "$tmp_dir/payload"
...
SHEPHERD_OS=Linux SHEPHERD_ARCH=x86_64 SHEPHERD_VERSION=6.4.9 \
...
expect_eq "$("$install_dir/shepherd")" 'shepherd fixture 6.4.9' 'installed executable'

printf 'do-not-replace\n' >"$install_dir/shepherd"
if SHEPHERD_OS=Linux SHEPHERD_ARCH=x86_64 SHEPHERD_VERSION=6.4.9 \
  SHEPHERD_RELEASE_BASE="file://$release_root" SHEPHERD_INSTALL_DIR="$install_dir" \
...
expect_eq "$("$install_dir/shepherd")" 'shepherd fixture 6.4.9' 'forced atomic replacement'
...
expect_eq "$("$dangling_dir/shepherd")" 'shepherd fixture 6.4.9' 'dangling symlink self-heal'
...
bad_dir="$release_root/download/v6.5.0"
...
bad_asset='shepherd-6.5.0-x86_64-unknown-linux-gnu.tar.gz'
(
...
if SHEPHERD_OS=Linux SHEPHERD_ARCH=x86_64 SHEPHERD_VERSION=6.5.0 \
